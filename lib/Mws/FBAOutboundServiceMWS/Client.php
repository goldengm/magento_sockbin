 ...
 *
 */
class Mws_FBAOutboundServiceMWS_Client implements Mws_FBAOutboundServiceMWS_Interface
{
  ...
  private function quoteAttributeValue($s) {
     ...
     * @return Mws_FBAOutboundServiceMWS_Model_GetPackageTrackingDetailsResponse Mws_FBAOutboundServiceMWS_Model_GetPackageTrackingDetailsResponse
     *
     * @throws Mws_FBAOutboundServiceMWS_Exception
     */
    public function getPackageTrackingDetails($request)
    {
        if (!$request instanceof Mws_FBAOutboundServiceMWS_Model_GetPackageTrackingDetailsRequest) {
            require_once ('Mws/FBAOutboundServiceMWS/Model/GetPackageTrackingDetailsRequest.php');
            $request = new Mws_FBAOutboundServiceMWS_Model_GetPackageTrackingDetailsRequest($request);
        }
        require_once ('Mws/FBAOutboundServiceMWS/Model/GetPackageTrackingDetailsResponse.php');
        return Mws_FBAOutboundServiceMWS_Model_GetPackageTrackingDetailsResponse::fromXML($this->_invoke($this->_convertGetPackageTrackingDetails($request)));
    }
    ...
    {
        if (!$request instanceof Mws_FBAOutboundServiceMWS_Model_ListAllFulfillmentOrdersRequest) {
            require_once ('Mws/FBAOutboundServiceMWS/Model/ListAllFulfillmentOrdersRequest.php');
            $request = new Mws_FBAOutboundServiceMWS_Model_ListAllFulfillmentOrdersRequest($request);
        }
        require_once ('Mws/FBAOutboundServiceMWS/Model/ListAllFulfillmentOrdersResponse.php');
        return Mws_FBAOutboundServiceMWS_Model_ListAllFulfillmentOrdersResponse::fromXML($this->_invoke($this->_convertListAllFulfillmentOrders($request)));
    }
    ...
    {
        if (!$request instanceof Mws_FBAOutboundServiceMWS_Model_GetFulfillmentPreviewRequest) {
            require_once ('Mws/FBAOutboundServiceMWS/Model/GetFulfillmentPreviewRequest.php');
            $request = new Mws_FBAOutboundServiceMWS_Model_GetFulfillmentPreviewRequest($request);
        }
        require_once ('Mws/FBAOutboundServiceMWS/Model/GetFulfillmentPreviewResponse.php');
        return Mws_FBAOutboundServiceMWS_Model_GetFulfillmentPreviewResponse::fromXML($this->_invoke($this->_convertGetFulfillmentPreview($request)));
    }
    ...
    {
        if (!$request instanceof Mws_FBAOutboundServiceMWS_Model_GetServiceStatusRequest) {
            require_once ('Mws/FBAOutboundServiceMWS/Model/GetServiceStatusRequest.php');
            $request = new Mws_FBAOutboundServiceMWS_Model_GetServiceStatusRequest($request);
        }
        require_once ('Mws/FBAOutboundServiceMWS/Model/GetServiceStatusResponse.php');
        return Mws_FBAOutboundServiceMWS_Model_GetServiceStatusResponse::fromXML($this->_invoke($this->_convertGetServiceStatus($request)));
    }
    ...
    {
        if (!$request instanceof Mws_FBAOutboundServiceMWS_Model_ListAllFulfillmentOrdersByNextTokenRequest) {
            require_once ('Mws/FBAOutboundServiceMWS/Model/ListAllFulfillmentOrdersByNextTokenRequest.php');
            $request = new Mws_FBAOutboundServiceMWS_Model_ListAllFulfillmentOrdersByNextTokenRequest($request);
        }
        require_once ('Mws/FBAOutboundServiceMWS/Model/ListAllFulfillmentOrdersByNextTokenResponse.php');
        return Mws_FBAOutboundServiceMWS_Model_ListAllFulfillmentOrdersByNextTokenResponse::fromXML($this->_invoke($this->_convertListAllFulfillmentOrdersByNextToken($request)));
    }
    ...
    {
        if (!$request instanceof Mws_FBAOutboundServiceMWS_Model_GetFulfillmentOrderRequest) {
            require_once ('Mws/FBAOutboundServiceMWS/Model/GetFulfillmentOrderRequest.php');
            $request = new Mws_FBAOutboundServiceMWS_Model_GetFulfillmentOrderRequest($request);
        }
        require_once ('Mws/FBAOutboundServiceMWS/Model/GetFulfillmentOrderResponse.php');
        return Mws_FBAOutboundServiceMWS_Model_GetFulfillmentOrderResponse::fromXML($this->_invoke($this->_convertGetFulfillmentOrder($request)));
    }
    ...
    {
        if (!$request instanceof Mws_FBAOutboundServiceMWS_Model_CancelFulfillmentOrderRequest) {
            require_once ('Mws/FBAOutboundServiceMWS/Model/CancelFulfillmentOrderRequest.php');
            $request = new Mws_FBAOutboundServiceMWS_Model_CancelFulfillmentOrderRequest($request);
        }
        require_once ('Mws/FBAOutboundServiceMWS/Model/CancelFulfillmentOrderResponse.php');
        return Mws_FBAOutboundServiceMWS_Model_CancelFulfillmentOrderResponse::fromXML($this->_invoke($this->_convertCancelFulfillmentOrder($request)));
    }
    ...
    {
        if (!$request instanceof Mws_FBAOutboundServiceMWS_Model_CreateFulfillmentOrderRequest) {
            require_once ('Mws/FBAOutboundServiceMWS/Model/CreateFulfillmentOrderRequest.php');
            $request = new Mws_FBAOutboundServiceMWS_Model_CreateFulfillmentOrderRequest($request);
        }
        require_once ('Mws/FBAOutboundServiceMWS/Model/CreateFulfillmentOrderResponse.php');
        return Mws_FBAOutboundServiceMWS_Model_CreateFulfillmentOrderResponse::fromXML($this->_invoke($this->_convertCreateFulfillmentOrder($request)));
    }
            ...
            do {
                try {
                        ...
                        switch ($httpStatus)
                        {
                            ...
                            case 500:
                            case 503:
                                require_once('Mws/FBAOutboundServiceMWS/Model/ErrorResponse.php');
                                $errorResponse = Mws_FBAOutboundServiceMWS_Model_ErrorResponse::fromXML($response['ResponseBody']);
                ...
                /* Rethrow on deserializer error */
                    ...
                    require_once ('Mws/FBAOutboundServiceMWS/Exception.php');
                    throw new Mws_FBAOutboundServiceMWS_Exception(array('Exception' => $e, 'Message' => $e->getMessage()));
                }

            } while ($shouldRetry);

        } catch (Mws_FBAOutboundServiceMWS_Exception $se) {
            throw $se;
        } catch (Exception $t) {
            throw new Mws_FBAOutboundServiceMWS_Exception(array('Exception' => $t, 'Message' => $t->getMessage()));
        }
    ...
    private function _reportAnyErrors($responseBody, $status, Exception $e =  null)
    {
        ...
        if (!is_null($responseBody) && strpos($responseBody, '<') === 0) {
            if (preg_match('@<RequestId>(.*)</RequestId>.*<Error><Code>(.*)</Code><Message>(.*)</Message></Error>.*(<Error>)?@mi',
                ...
                require_once ('Mws/FBAOutboundServiceMWS/Exception.php');
                $ex = new Mws_FBAOutboundServiceMWS_Exception(array ('Message' => $message, 'StatusCode' => $status, 'ErrorCode' => $code,
                ...
                require_once ('Mws/FBAOutboundServiceMWS/Exception.php');
                $ex = new Mws_FBAOutboundServiceMWS_Exception(array('Message' => 'Internal Error', 'StatusCode' => $status));
            }
        } else {
            require_once ('Mws/FBAOutboundServiceMWS/Exception.php');
            $ex = new Mws_FBAOutboundServiceMWS_Exception(array('Message' => 'Internal Error', 'StatusCode' => $status));
     ...
     * @throws Mws_FBAOutboundServiceMWS_Exception if maximum number of retries has been reached
     */
