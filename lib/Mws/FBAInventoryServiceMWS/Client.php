...
/**
 *  @see Mws_FBAInventoryServiceMWS_Interface
 */
require_once ('Mws/FBAInventoryServiceMWS/Interface.php');
 ...
 *
 */
class Mws_FBAInventoryServiceMWS_Client implements Mws_FBAInventoryServiceMWS_Interface
{
  ...
  private function quoteAttributeValue($s) {
     ...
     * @return Mws_FBAInventoryServiceMWS_Model_ListInventorySupplyByNextTokenResponse FBAInventoryServiceMWS_Model_ListInventorySupplyByNextTokenResponse
     *
     * @throws Mws_FBAInventoryServiceMWS_Exception
     */
    public function listInventorySupplyByNextToken($request)
    {
        if (!$request instanceof Mws_FBAInventoryServiceMWS_Model_ListInventorySupplyByNextTokenRequest) {
            require_once ('Mws/FBAInventoryServiceMWS/Model/ListInventorySupplyByNextTokenRequest.php');
            $request = new Mws_FBAInventoryServiceMWS_Model_ListInventorySupplyByNextTokenRequest($request);
        }
        require_once ('Mws/FBAInventoryServiceMWS/Model/ListInventorySupplyByNextTokenResponse.php');
        return Mws_FBAInventoryServiceMWS_Model_ListInventorySupplyByNextTokenResponse::fromXML($this->_invoke($this->_convertListInventorySupplyByNextToken($request)));
    }
     ...
     * @return Mws_FBAInventoryServiceMWS_Model_ListInventorySupplyResponse FBAInventoryServiceMWS_Model_ListInventorySupplyResponse
     *
     * @throws Mws_FBAInventoryServiceMWS_Exception
     */
    public function listInventorySupply($request)
    {
        if (!$request instanceof Mws_FBAInventoryServiceMWS_Model_ListInventorySupplyRequest) {
            require_once ('Mws/FBAInventoryServiceMWS/Model/ListInventorySupplyRequest.php');
            $request = new Mws_FBAInventoryServiceMWS_Model_ListInventorySupplyRequest($request);
        }
        require_once ('Mws/FBAInventoryServiceMWS/Model/ListInventorySupplyResponse.php');
        return Mws_FBAInventoryServiceMWS_Model_ListInventorySupplyResponse::fromXML($this->_invoke($this->_convertListInventorySupply($request)));
    }
     ...
     * @return Mws_FBAInventoryServiceMWS_Model_GetServiceStatusResponse FBAInventoryServiceMWS_Model_GetServiceStatusResponse
     *
     * @throws Mws_FBAInventoryServiceMWS_Exception
     */
    public function getServiceStatus($request)
    {
        if (!$request instanceof Mws_FBAInventoryServiceMWS_Model_GetServiceStatusRequest) {
            require_once ('Mws/FBAInventoryServiceMWS/Model/GetServiceStatusRequest.php');
            $request = new Mws_FBAInventoryServiceMWS_Model_GetServiceStatusRequest($request);
        }
        require_once ('Mws/FBAInventoryServiceMWS/Model/GetServiceStatusResponse.php');
        return Mws_FBAInventoryServiceMWS_Model_GetServiceStatusResponse::fromXML($this->_invoke($this->_convertGetServiceStatus($request)));
    }
            ...
            do {
                try {
                        ...
                        switch ($httpStatus)
                        {
                            ...
                            case 500:
                            case 503:
                                require_once('Mws/FBAInventoryServiceMWS/Model/ErrorResponse.php');
                                $errorResponse = Mws_FBAInventoryServiceMWS_Model_ErrorResponse::fromXML($response['ResponseBody']);
                ...
                /* Rethrow on deserializer error */
                    ...
                    require_once ('Mws/FBAInventoryServiceMWS/Exception.php');
                    throw new Mws_FBAInventoryServiceMWS_Exception(array('Exception' => $e, 'Message' => $e->getMessage()));
                }

            } while ($shouldRetry);

        } catch (Mws_FBAInventoryServiceMWS_Exception $se) {
            throw $se;
        } catch (Exception $t) {
            throw new Mws_FBAInventoryServiceMWS_Exception(array('Exception' => $t, 'Message' => $t->getMessage()));
        }
    ...
    private function _reportAnyErrors($responseBody, $status, Exception $e =  null)
    {
        ...
        if (!is_null($responseBody) && strpos($responseBody, '<') === 0) {
            if (preg_match('@<RequestId>(.*)</RequestId>.*<Error><Code>(.*)</Code><Message>(.*)</Message></Error>.*(<Error>)?@mi',
                ...
                require_once ('Mws/FBAInventoryServiceMWS/Exception.php');
                $ex = new Mws_FBAInventoryServiceMWS_Exception(array ('Message' => $message, 'StatusCode' => $status, 'ErrorCode' => $code,
                ...
                require_once ('Mws/FBAInventoryServiceMWS/Exception.php');
                $ex = new Mws_FBAInventoryServiceMWS_Exception(array('Message' => 'Internal Error', 'StatusCode' => $status));
            }
        } else {
            require_once ('Mws/FBAInventoryServiceMWS/Exception.php');
            $ex = new Mws_FBAInventoryServiceMWS_Exception(array('Message' => 'Internal Error', 'StatusCode' => $status));
     ...
     * @throws Mws_FBAInventoryServiceMWS_Exception if maximum number of retries has been reached
     */
