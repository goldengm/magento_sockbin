...
/**
 * Solidcommerce resource catalog collection model
 *
 * @category    Sockbin
 * @package     Sockbin_Solidcommerce
 * @author      Magento Core Team <core@magentocommerce.com>
 */
abstract class Sockbin_Solidcommerce_Model_Resource_Catalog_Abstract extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
    ...
    protected function _prepareObject(array $row)
    {
        ...
        $attributeValue = Mage::getModel('catalog/product')->load($row[$this->getIdFieldName()])->getAttributeText('color');
        $entity = new Varien_Object();
        $entity->setId($row[$this->getIdFieldName()]);
        $entity->setUrl($this->_getEntityUrl($row, $entity));
        $entity->setName($_product['name']);
        // 2014-12-10
        // Dabb Media added if statements 
        // to prevent errors seen in log
        if (!empty($_product['upc'])) {
            $entity->setUpc($_product['upc']);
        }
        $entity->setMsrp(number_format($_product['msrp'], 2));
        $entity->setPrice(number_format($_product['price'], 2));
        $entity->setImage($_product['image']);
        $entity->setImage1((!empty($_product['images'][1]['file'])) ? $_product['images'][1]['file']: '');
        $entity->setImage2((!empty($_product['images'][2]['file'])) ? $_product['images'][2]['file']: '');
        $entity->setImage3((!empty($_product['images'][3]['file'])) ? $_product['images'][3]['file']: '');
        $entity->setSku($_product['sku']);
        $entity->setDescription($_product['description']);
        if (!empty($_product['quantity'])) {
            $entity->setInventory($_product['quantity']);
        }
        if (!empty($_product['qty'])) {
            $entity->setInventory($_product['qty']);
        }
        $entity->setAttributeName((!empty($attributeValue)) ? 'Color': '');
        $entity->setAttributeColor((!empty($attributeValue)) ? $attributeValue: '');
        return $entity;
    }
     ...
     *
     * @param string $attributeCode
     * @return Sockbin_Solidcommerce_Model_Resource_Catalog_Abstract
     */
