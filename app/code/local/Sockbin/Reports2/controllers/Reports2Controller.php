 ...
 */
class Sockbin_Reports2_Reports2Controller extends Mage_Adminhtml_Controller_Report_Abstract
{
    /**
...
//    }
    ...
    public function quantitiesAction()
    {
        $this->_title($this->__('Reports'))
             ->_title($this->__('Products'))
             ->_title($this->__('Quantities'));

        $this->_initAction()
            ->_setActiveMenu('report2/product/quantities')
            ->_addBreadcrumb(Mage::helper('reports2')->__('Quantities'), Mage::helper('reports2')->__('Quantities'))
            ->_addContent($this->getLayout()->createBlock('adminhtml/report2_product_quantities'))
            ->renderLayout();
    }

    /**
     * Export quantities products report to CSV format
     *
     */
    public function exportQuantitiesCsvAction()
    {
        $fileName   = 'products_quantities.csv';
        $content    = $this->getLayout()->createBlock('adminhtml/report2_product_quantities_grid')
    ...
    /**
     * Export quantities products report to XML format
     *
     */
    public function exportQuantitiesExcelAction()
    {
        $fileName   = 'products_quantities.xml';
        $content    = $this->getLayout()->createBlock('adminhtml/report2_product_quantities_grid')
            ->setSaveParametersInSession(true)
            ->getExcel($fileName);
     ...
     *
     */
    public function lessthanzeroAction()
    {
        $this->_title($this->__('Reports'))
             ->_title($this->__('Products'))
             ->_title($this->__('Less than Zero'));

        $this->_initAction()
            ->_setActiveMenu('report2/product/lessthanzero')
            ->_addBreadcrumb(Mage::helper('reports2')->__('Lessthanzero'), Mage::helper('reports2')->__('Lessthanzero'))
            ->_addContent($this->getLayout()->createBlock('adminhtml/report2_product_lessthanzero'))
            ->renderLayout();
    }

    /**
     * Export lessthanzero products report to CSV format
     *
     */
    public function exportLessthanzeroCsvAction()
    {
        $fileName   = 'products_lessthanzero.csv';
        $content    = $this->getLayout()->createBlock('adminhtml/report2_product_lessthanzero_grid')
            ->setSaveParametersInSession(true)
            ->getCsv();
    ...
    /**
     * Export lessthanzero products report to XML format
     *
     */
    public function exportLessthanzeroExcelAction()
    {
        $fileName   = 'products_lessthanzero.xml';
        $content    = $this->getLayout()->createBlock('adminhtml/report2_product_lessthanzero_grid')
            ->setSaveParametersInSession(true)
            ->getExcel($fileName);
    ...
    protected function _isAllowed()
    {
        switch ($this->getRequest()->getActionName()) {
            ...
            case 'sold':
                ...
                break;
            case 'quantities':
                return Mage::getSingleton('admin/session')->isAllowed('report/products/quantities');
                break;
            default:
