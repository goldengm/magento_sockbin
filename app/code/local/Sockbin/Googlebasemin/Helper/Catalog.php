 ...
 */
class Sockbin_Googlebasemin_Helper_Catalog extends Mage_Core_Helper_Abstract
{
    const XML_PATH_SITEMAP_VALID_PATHS = 'general/file/sitemap_generate_valid_paths';
    const XML_PATH_GOOGLEBASEMIN_VALID_PATHS = 'general/file/googlebasemin_generate_valid_paths';
    ...
    /**
     * Get list valid paths for generate a googlebasemin txt file
     *
     * @return array
     */
    public function getGooglebaseminValidPaths()
    {
        $path = Mage::getStoreConfig(self::XML_PATH_GOOGLEBASEMIN_VALID_PATHS);
        /** @var $helper Mage_Core_Helper_Data */
        $helper = Mage::helper('core');
        $path = array_merge($path, $helper->getPublicFilesValidPath());
        return $path;
