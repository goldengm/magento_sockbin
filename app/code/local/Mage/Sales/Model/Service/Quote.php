...
class Mage_Sales_Model_Service_Quote
{
    ...
    public function submitOrder()
    {
        ...
        foreach ($quote->getAllItems() as $item) {
            ...
            $optionData = $item->getOrigData();
            $optionSku = $optionData['sku'];
            $optionQty = $optionData['qty'];
            $tmp_product = $item->getProduct();
            $product_id = $tmp_product->getId();
            ...
            foreach ($product->getOptions() as $o) {
                ...
                foreach($p as $v) {
                    ...
                    $values[$v->getId()]['price_type']= 'percent';
                    $values[$v->getId()]['sku']= $loop_sku;
                    if ($this->endswith($optionSku, $loop_sku) !== false) {
                        $values[$v->getId()]['qty']= $v->getQty() - $optionQty;
                    } else {
                        $values[$v->getId()]['qty']= $v->getQty();
                    }
                    $v->setValues($values);
                    $v->saveValues();
                }
