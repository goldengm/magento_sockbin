...
class Mirasvit_FeedExport_Helper_Format extends Mage_Core_Helper_Abstract
{
    ...
    public function expandFormat($object)
    {
        if ($object->getType() == 'csv' || $object->getType() == 'txt') {
            ...
            if (isset($format['mapping'])) {
                if (!isset($format['include_second_header'])) {
                    $format['include_second_header'] = 0;
                }

                if (!isset($format['mapping']['second_header'])) {
                    $format['mapping']['second_header'][0] = '';
                }

                $count = count($format['mapping']['header']) - count($format['mapping']['second_header']);
                if ($count > 0) {
                    for ($i = 0; $count > 0; ++$i, --$count) {
                        $format['mapping']['second_header'][] = '';
                    }
                }
            }
            ...
            for ($i = 0; $i < count($mapping['header']); ++$i) {
                $item = array(
                    ...
                    'second_header' => $mapping['second_header'][$i],
    ...
    public function csvFormatToXml($format)
    {
        ...
        if ($format['extra_header']) {
            if ($delimiter == "\t") {
                $xml .= str_replace('\t', "\t", $format['extra_header']).PHP_EOL;
            } else {
        ...
        if ($format['include_second_header']) {
            $secondColumns = array();
            if (isset($format['mapping'])) {
                foreach ($format['mapping']['second_header'] as $indx => $secondHeader) {
                    $secondColumns[] = $secondHeader;
                }
            }
            $xml .= implode($delimiter, $secondColumns).PHP_EOL;
        }
