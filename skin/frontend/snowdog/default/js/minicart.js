...
Minicart.prototype = {
    ...
    init: function() {
        var cart = this;
        // bind remove event
        $j(cart.selectors.itemRemove).unbind('click.minicart').bind('click.minicart', function(e) {
            e.preventDefault();
            cart.removeItem($j(this));
        });

        // bind update qty event
        $j(cart.selectors.inputQty)
            ...
            .bind('blur.minicart', function() {
                cart.revertInvalidValue(cart);
            });

        $j(cart.selectors.quantityButtonClass)
            .unbind('click.quantity')
            .bind('click.quantity', function() {
                cart.processUpdateQuantity(cart);
        });
    },
